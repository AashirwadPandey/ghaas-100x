import OfficeMap from "../components/OfficeMap";
import LangText from "../components/LangText";

async function fetchOffices() {
  const base = process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:3001";
  try {
    const res = await fetch(`${base}/api/offices`, { cache: 'no-store' });
    if (!res.ok) return [] as any[];
  const data = await res.json();
  return data.data || [];
  } catch {
    return [] as any[];
  }
}

export default async function Page() {
  const offices = await fetchOffices();
  return (
    <main>
      <h1><LangText en="GHaaS Prototype" ne="जीएचएएएस प्रोटोटाइप" /></h1>
      <p><LangText en="Explore nearby government offices and services." ne="नजिकैका सरकारी कार्यालयहरू र सेवाहरू अन्वेषण गर्नुहोस्।" /></p>
      <div style={{ height: 360, marginTop: 16 }}>
        {/* Client map renders markers when available */}
        <OfficeMap pins={(offices || []).map((o: any) => ({ id: String(o.id), name: o.name, lat: o.lat, lng: o.lng }))} />
      </div>
    </main>
  );
}
